...
public class PersistAddItemsRequestHandler implements DNRequestHandler{
	
	Logger m_logger = Logger.getLogger(PersistAddItemsRequestHandler.class);
	...
	public void handleRequest(DNRequestContext requestContext, InputStream is,
		...
		while(!m_stop) {
			...
			try {
				...
				is.read(buff,0,1);
				request = new PersistAddItemsRequest();
				request.read(is);
			} catch (IOException e) {
				m_stop = true;
				m_logger.debug("Connection closed from remote client.");
			}
		}
	...
	private int processRequest(NameNodeClient nn_client,long startItemID, DNRequestContext requestContext,
			InputStream is, OutputStream os, PersistAddItemsRequest request,
